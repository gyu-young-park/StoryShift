...
func TestMarkdownImageeMatcher(t *testing.T) {
	pictures := markdownImageMatcher(MARKDONW_CONTENT_WITH_IMAGE)
...
func TestReplaceAllMarkdownImageUrl(t *testing.T) {
	replacedMarkdownContent := replaceAllMarkdownImageUrl("image-prefix", MARKDONW_CONTENT_WITH_IMAGE)
	pictures := markdownImageMatcher(replacedMarkdownContent)
	assert.Equal(t, "image-prefix-0", pictures[0])
}

func TestDownloadImageWithUrl(t *testing.T) {
	images := markdownImageMatcher(MARKDONW_CONTENT_WITH_IMAGE)
